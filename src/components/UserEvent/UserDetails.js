...
export const UserDetails = ({ handle, getUserDetails, follows }) => {
  ...
  return (
    <div className='my-blogs__user-card'>
      ...
      {user.handle === sessionStorage.getItem('userHandle') && (
        <Link to='edit'>
          <button className='button button-editProfile'>Edit Profile</button>
        </Link>
      )}
    </div>
  )
